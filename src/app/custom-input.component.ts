...
import { inputItem } from "./input.model";
...
export class CustomInputComponent implements OnInit {
  @Input() control: inputItem = <inputItem>{
    type: 'calendar',
    className: '',
    controlName: 'test',
    placeholder: 'My Keyword',
    isRequired: true
  };
  ...
  changInputStatus(target) {
    ...
    if (isNumberInput) {
      if (target.value) {
        if (target.value < this.control.minNumber) {
          target.value = this.control.minNumber;
          this.control.model = this.control.minNumber;
        }

        // make sure the input number is integer
        if (this.control.isIntegerOnly) {
          this.control.model = Math.floor(this.control.model);
        }
      } else {
        // to fix number that start with e, eg: e12
        target.value = '';
      }
    }
  }
  ...
  focusCalendar(e) {
    ...
    setTimeout(() => {
      ...
      const isOutsideModal =
        dialog && dialog.clientHeight <= datepicker.clientHeight;
  ...
  updateLeftPos(rect, datepicker) {
    ...
    datepicker.setAttribute(
      "style",
      `left: -${left}px!important; z-index: 1002`
    );
  }

  updateTopPos(rect, datepicker) {
    const offsetTop = this.utilService.getOffsetTop(
      datepicker,
      "ui-dialog-content"
    );
    ...
    datepicker.setAttribute(
      "style",
      `top: -${offsetTop}px!important; z-index: 1002`
    );
